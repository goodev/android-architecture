...
class TasksPresenter(private val tasksRepository: TasksRepository,
     ...
     */
    private fun loadTasks(forceUpdate: Boolean, showLoadingUI: Boolean) = coroutineScope.launch {
        if (showLoadingUI) {
    ...
    override fun completeTask(completedTask: Task) = coroutineScope.launch {
        tasksRepository.completeTask(completedTask)
        tasksView.showTaskMarkedComplete()
        loadTasks(false, false)
    }

    override fun activateTask(activeTask: Task) = coroutineScope.launch {
        tasksRepository.activateTask(activeTask)
        tasksView.showTaskMarkedActive()
        loadTasks(false, false)
    }

    override fun clearCompletedTasks() = coroutineScope.launch {
